...
function AddRescue({ navigation, route }) {
    const [state, setState] = useState({
        ...
        uploading: false,
        confirmModal: false,
        addRescueModal: false
    })

    const handleNext = () => {
        const { rescueList } = state;
        const programAdmin = route.params?.programAdmin
        const inspectorList = route.params?.inspectorList
        navigation.navigate("ConfirmTeam", { programAdmin, inspectorList, rescueList });
    }
    ...
    const handleOpenModal = () => {
        setState(pre => ({ ...pre, addRescueModal: true }))
    }

    const handleCloseModal = () => {
        setState(pre => ({ ...pre, addRescueModal: false }))
    }

    const handleModalClick = (checked) => {
        const { name, mobilePhone, leadRole, profile_picture, rescueList } = state;
        const checkLeadRole = rescueList.some(e => e.leadRole === true)
        ...
        const newlist = rescueList
        setState(pre => ({
            ...pre, addRescueModal: false, rescueList: [...newlist, list],
            name: "", mobilePhone: "", leadRole: false, profile_picture: ""
        }))
    }


    const { name, mobilePhone, leadRole, profile_picture, rescueList, addRescueModal, confirmModal } = state;
        ...
        <View style={styles.container}>
            <View style={styles.viewContainer}>
                ...
                <View style={styles.centerContainer}>
                    ...
                    <Text style={styles.subheading}>Add and Confirm Rescue Personnel</Text>
                    <ScrollView showsVerticalScrollIndicator={false} style={styles.scrollView}>
                        <Text style={[styles.selectTeam]}>Select and confirm team members authorized to conduct personal fall protection rescues.</Text>
                        {
                            rescueList.map((inspector, index) => (
                                        ...
                                        <Text style={styles.nametext}>{inspector.name}</Text>
                                        {
                                            inspector.leadRole ?
                                                <View style={styles.subView}>
                                                    <SvgXml xml={group} />
                                                    <Text style={styles.leadtext}>{'Rescue Team Lead'}</Text>
                                                </View>
                                                :
                                                <Text style={styles.addtext}>{"Rescuer"}</Text>
                                        }
                                    </View>
                        ...
                        <TouchableOpacity style={styles.addContainer} onPress={handleOpenModal}>
                            ...
                            <Text style={styles.addtext}>Add Rescuer</Text>
                        </TouchableOpacity>
                    </ScrollView>
                </View>
            ...
            <AddInspectorModal
                ...
                headerTitle={"ADD RESCUER"}
                contentType={'rescuer'}
                checkboxText={'Rescue Team Lead'}
                checkboxDescription={'The Rescue Team Lead is responsible for overseeing rescue functions.  Please refer to ANSI Z359 for additional information.'}
                buttonBottomText={'By hitting confirm, you verify that this person is authorized to perform rescues per the requirements set forth in ANSI Z359.'}
                buttonTitle={"CONFIRM"}
